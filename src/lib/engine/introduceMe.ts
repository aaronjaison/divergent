import { capPerArtist } from "./diversity";
import {
  distributeSeed,
  interleave,
  separateAdjacent,
...
export function buildFamous(input: FamousInput): GeneratedPlaylist {
  const { artist, constraints } = input;
  ...
  if (tracks.length < constraints.targetLength) {
    warnings.push(
      `Only ${tracks.length} well-known tracks were found for ${artist.name}.`,
    );
  }
